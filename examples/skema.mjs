import pkg, { HentLektionInfo, HentSkemaUge } from '../lib/index.js';
...
HentSkemaUge(user, lectioHelper, 2021, 3).catch((error) => {
  console.log(error);
}).then(async (timetable) => {
  console.log(JSON.stringify(timetable, null, 2));
  let detailedLessonInfo = await HentLektionInfo(user, lectioHelper, '43394768400');
